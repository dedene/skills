#!/usr/bin/env python3
"""Fetch and inspect YouTube transcripts with yt-dlp.
...
DEFAULT_OUTPUT_DIR = "youtube-transcripts"
...
def yt_dlp_base_args(args: argparse.Namespace) -> list[str]:
    ...
    cookies_from_browser = args.cookies_from_browser or os.environ.get(
...
def safe_video_id(info: dict[str, Any]) -> str:
    ...
    if not re.fullmatch(r"[A-Za-z0-9_-]{6,}", video_id):
        fail("could not determine a safe YouTube video id")
    return video_id
...
def find_vtt_file(video_dir: Path, video_id: str, languages: str) -> Path | None:
    ...
    candidates = [path for path in candidates if "live_chat" not in path.name]
    if not candidates:
        return None

    patterns = language_patterns(languages)
    for lang in patterns:
        prefix = lang.rstrip("*").rstrip(".")
        if not prefix:
            continue
...
def parse_vtt(path: Path) -> list[dict[str, Any]]:
    ...
    return collapse_rolling_captions(cues)


def caption_delta(previous: str, current: str) -> str:
    if not previous:
        return current
...
def collapse_rolling_captions(cues: list[dict[str, Any]]) -> list[dict[str, Any]]:
    collapsed: list[dict[str, Any]] = []
    previous_text = ""
    for cue in cues:
        delta = caption_delta(previous_text, cue["text"])
        previous_text = cue["text"]
        if not delta:
            continue
        collapsed_cue = dict(cue)
        collapsed_cue["text"] = delta
        if collapsed and collapsed[-1]["text"] == collapsed_cue["text"]:
            continue
        collapsed.append(collapsed_cue)
    return collapsed


def write_markdown(path: Path, info: dict[str, Any], cues: list[dict[str, Any]]) -> None:
    title = info.get("title") or info.get("id") or "YouTube transcript"
...
def write_plain_text(path: Path, cues: list[dict[str, Any]]) -> None:
    path.write_text(plain_text_from_cues(cues), encoding="utf-8")
...
def command_list(args: argparse.Namespace) -> None:
    info = load_info(args.url, args)
    manual = sorted((info.get("subtitles") or {}).keys())
    automatic = sorted((info.get("automatic_captions") or {}).keys())

    print(f"video_id: {info.get('id') or ''}")
    print(f"title: {info.get('title') or ''}")
    print(f"channel: {info.get('uploader') or info.get('channel') or ''}")
    print(f"manual_subtitle_count: {len(manual)}")
    print(f"automatic_caption_count: {len(automatic)}")
    print("")
    print("manual_subtitles:")
    for language in manual[: args.limit]:
        print(f"- {language}")
    if len(manual) > args.limit:
        print(f"- ... {len(manual) - args.limit} more")
    print("")
    print("automatic_captions:")
    for language in automatic[: args.limit]:
        print(f"- {language}")
    if len(automatic) > args.limit:
        print(f"- ... {len(automatic) - args.limit} more")
...
def command_fetch(args: argparse.Namespace) -> None:
    ...
    if not subtitle_file:
        compact = compact_metadata(info)
        write_json(video_dir / "metadata.compact.json", compact)
        fail(
            f"no VTT subtitles found for {video_id}. Run list to inspect available languages: {video_dir}"
        )
    ...
    if subtitle_file.resolve() != transcript_vtt.resolve():
        transcript_vtt.write_text(subtitle_file.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
    ...
    write_markdown(transcript_md, info, cues)
    write_plain_text(transcript_txt, cues)
    write_json(
        video_dir / "metadata.compact.json",
        compact_metadata(info, transcript_file=transcript_md, plain_text_file=transcript_txt),
    )
    ...
    print(f"selected_subtitle: {subtitle_file}")
    print(f"cue_count: {len(cues)}")
    print(f"artifact_dir: {video_dir}")
    print(f"metadata: {video_dir / 'metadata.compact.json'}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fetch and inspect YouTube transcripts.")
    ...
    list_parser = subparsers.add_parser("list", help="List available subtitle languages.")
    list_parser.add_argument("url", help="YouTube URL or video id")
    list_parser.add_argument("--limit", type=int, default=80, help="Maximum language codes per section.")
    add_cookie_args(list_parser)
    list_parser.set_defaults(func=command_list)

    fetch_parser = subparsers.add_parser("fetch", help="Fetch subtitles and write transcript artifacts.")
    fetch_parser.add_argument("url", help="YouTube URL or video id")
    fetch_parser.add_argument("--languages", default=DEFAULT_LANGUAGES, help=f"yt-dlp sub language spec. Default: {DEFAULT_LANGUAGES}")
    fetch_parser.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR, help=f"Output root. Default: {DEFAULT_OUTPUT_DIR}")
    fetch_parser.add_argument("--refresh", action="store_true", help="Re-download subtitles even when cached.")
    fetch_parser.add_argument("--preview-lines", type=int, default=5, help="Number of cues to print after fetch.")
    add_cookie_args(fetch_parser)
    ...
    excerpt_parser.add_argument("--start", required=True, help="Start time, e.g. 00:10:00 or seconds.")
    excerpt_parser.add_argument("--end", required=True, help="End time, e.g. 00:12:30 or seconds.")
